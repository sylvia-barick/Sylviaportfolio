...
const EducationTimeline = () => {
  ...
  return (
    <div>
      <h3 className="text-3xl font-playfair font-bold text-center mb-12 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
        Educational Journey
      </h3>
        ...
        {educationData.map((item, index) => (
          <div key={index} className={`relative flex items-center mb-12 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
            ...
            <div className={`ml-12 md:ml-0 md:w-5/12 ${index % 2 === 0 ? 'md:mr-auto md:pr-8' : 'md:ml-auto md:pl-8'}`}>
              <div className={`bg-gray-700/30 backdrop-blur-sm rounded-2xl p-6 border border-gray-600/30 transform transition-all duration-500 hover:scale-105 hover:border-purple-400/50 ${
                item.status === 'current' ? 'ring-2 ring-purple-400/50' : ''
              }`}>
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-sm font-inter text-purple-400 bg-purple-400/20 px-3 py-1 rounded-full">
                    {item.year}
                  </span>
                  {item.status === 'current' && (
                    <span className="text-xs font-inter text-green-400 bg-green-400/20 px-2 py-1 rounded-full">
                      Current
                    </span>
                  )}
                </div>
                <h4 className="text-xl font-inter font-bold text-white mb-2">
                  {item.title}
                </h4>
                <p className="text-purple-300 font-inter font-medium mb-2">
                  {item.institution}
                </p>
